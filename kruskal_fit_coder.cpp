...
void addEdge(vector<vector<pair<int,int>>>& graph, vector<tuple<int,int,int>>& allEdges,
    int u, int v, int w)
{
    graph[u].push_back({v,w});//make_pair(v,w));
    graph[v].push_back({u,w});//make_pair(u,w));
    allEdges.push_back({u,v,w});//make_tuple(u,v,w));
}
