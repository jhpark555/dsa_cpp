...
int read_tree(node *nodes[],int num_lines){
    ...
    for(i=0; i< num_lines; i++){
        ...
        if(parent_node==NULL){
            ...
            free(parent_name);
        parent_node->children = (node**)malloc_safe(sizeof(node) * num_children);
...
int main()
{
    ...
    int n,d,num_nodes;
    node **nodes=(node**)malloc_safe(sizeof(node) * MAX_NODES);
    printf("num_cases:");
    scanf("%d", &num_cases);
